...
export class Base<Data> {
	...
	static withSave<Data extends object, Save extends object>(
		savePath: keyof PKGameData,
		emptySave: Save
	) {
		// @ts-ignore
		...
		return BaseWithSave as any as new (source: Data, game: PKGame) => Data &
			BaseWithSave &
			ToRefs<Save>;
	}
	static make<Data>() {
...
export class Multiplier
	extends Base.make<MultiplierData>()
	implements MultiplierImpl
{
	...
	get effectText(): string {
		if (this.effectTextTemplate) {
			...
			return `x${KMBFormat(this.multiplier)} ${this.name}`;
		}
	}
...
export class Job
	extends Base.withSave<JobData, JobSave>("jobs", jobEmptySave)
	implements JobImpl
{
...
export class Skill extends Base.withSave<SkillData, SkillSave>(
	"skills",
	skillEmptySave
) {
	get currentExpGain(): multi {
